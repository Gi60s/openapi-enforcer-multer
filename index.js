...
module.exports = function (enforcer, upload, { extension = 'x-multer' } = {}) {
        ...
        // there is a multer middleware
          ...
          multer.middleware(req, res, function (err) {
            ...
            Object.keys(req.files).forEach(key => {
              ...
              if (prop.type === 'array') {
                req.body[key] = files.map(file => file.buffer)
              } else if (files.length) {
                req.body[key] = files[files.length - 1].buffer
              }
            })
            next()
          })
